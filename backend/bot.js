...
async function postToChannelInternal(house) {
  try {
    ...
    const titleCaption = `📢 *${escapeMarkdown(house.title)}*`;
    
    let detailsCaption = `📢 *${escapeMarkdown(house.title)}*

💰 Narx: ${house.currency === 'USD' ? '$' : ''}${house.price}${house.currency === 'UZS' ? " so'm" : ''}${house.address ? `\n📍 Manzil: ${escapeMarkdown(house.address)}` : ''}${house.location_type ? `\n🏢 Joylashuv: ${house.location_type === 'shahar-atrofi' ? 'Shaxar atrofi' : escapeMarkdown(house.location_type)}` : ''}${house.house_type ? `\n🏠 Turi: ${house.house_type === 'uy' ? 'Dom' : escapeMarkdown(house.house_type)}` : ''}${house.area ? `\n📐 Maydon: ${house.area} m²` : ''}${house.rooms ? `\n🏠 Xonalar: ${house.rooms}` : ''}${house.floor ? `\n🏢 Qavat ${house.floor} Umumiy Qavat ${house.total_floors}` : ''}${house.condition ? `\n🔧 Holati: ${escapeMarkdown(house.condition)}` : ''}${house.furniture ? `\n🪑 Uy jihozlar: ${escapeMarkdown(house.furniture)}` : ''}${house.utilities ? `\n⚡ Komunal: ${escapeMarkdown(house.utilities)}` : ''}${house.year_built ? `\n📅 Qurilgan yili: ${house.year_built}` : ''}${house.garage ? `\n🚗 Garaj: ${house.garage === 'bor' ? 'Bor' : 'Yo\'q'}` : ''}${house.description ? `\n📝 Tavsif: ${escapeMarkdown(house.description)}` : ''}${house.created_at ? `\n📅 Joylangan: ${new Date(house.created_at + 'Z').toLocaleDateString('uz-UZ')} ${new Date(house.created_at + 'Z').toLocaleTimeString('uz-UZ')}` : ''}${house.expiration_date ? `\n⏰ Tugash sanasi: ${new Date(house.expiration_date).toLocaleDateString('uz-UZ')}` : '\n⏰ Muddati: Cheksiz'}

📞 Telefon: [${escapeMarkdown(house.phone)}](tel:${escapeMarkdown(house.phone)})
👤 E'lon qo'shgan: ${escapeMarkdown(house.telegram_username || house.username || 'Foydalanuvchi').replace(/^@/, '')}`;
